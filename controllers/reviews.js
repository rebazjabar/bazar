...
exports.getProductReviews = async function (req, res) {
  ...
  try {
    ...
    // 🛠️ Handle cases where Authorization header is missing
    let tokenData = null;
    ...
    const reviews = await Review.aggregate([
      ...
      },
      {
        $addFields: {
          sortId: {
            $cond: [
              { $and: [tokenData, { $eq: ['$user', new mongoose.Types.ObjectId(tokenData?.id)] }] },
              0,
              1,
            ],
          },
    ...
    // 🛠️ Fetch user names for reviews
    ...
    for (const review of reviews) {
      ...
      if (review.userName !== user.name) {
        ...
        await Review.updateOne({ _id: review._id }, { $set: { userName: user.name } }, { session });
      }
